...
class Deeplearning:
    ...
    def plot_reward(self,reward, i,filename,plot_name):
        ...
        plt.plot(Train_list,reward)
        plt.title('训练过程',fontsize=15)
        plt.xlabel('回合/k',fontsize=15)
    ...
    def train(self, save_step, episodes):
        ...
        for i in range(len(self.reward)):
            self.reward[i] /= 100 # to present results
        self.plot_reward(self.reward, episodes, "Score", "累计奖励")
        self.plot_reward(self.reward_buffer, episodes, "Reward", "单步奖励")
