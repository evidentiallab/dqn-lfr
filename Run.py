...
def ILP_solution(model):                ## 1.ILP
    ILP_path = []
    weight_ILP = 0
    cost, res,time = LoadBalancingSteinerTree(model, 6).main(plot=False)
    for i in res:
...
def Heuristic_solution(model):             ## 3.Heuristic
    ...
    length, path = k_shortest_paths(G_contrast_Positive, model.src_node, model.dst_node,  2)
...
if __name__ == '__main__':
    graph_path = 'DQN/Graph_and_Net_Model/model/23021509_55/graph_model.pkl'
    ...
    if Test_one:
        ...
        net_path = "DQN/Graph_and_Net_Model/model/23021509_55/percent_1/policy_net-45000.pth"
        ...
        DQN_model.plot_path(directory,p4, 'DQN_1.svg')
    ...
    if Test:
        ...
        for train_episodes in range(Network_pth, Train_episodes, Network_pth):
            net_path = 'DQN/Graph_and_Net_Model/model/23021509_55/percent_1/policy_net-' + str(train_episodes) + '.pth'
